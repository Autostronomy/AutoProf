...
def Orthogonal_Sample(IMG, results, options):
    ...
    pa = (options['orthsample_pa']*np.pi/180) if 'orthsample_pa' in options else results['init pa']
    if 'orthsample_parallel' in options and options['orthsample_parallel']:
        pa += np.pi/2
    dat = IMG - results['background']
    zeropoint = options['zeropoint'] if 'zeropoint' in options else 22.5

    if 'prof data' in results:
        Rproflim = results['prof data']['R'][-1]/options['pixscale']
    else:
        Rproflim = min(IMG.shape)/2
    
    R = [0]
    while R[-1] < Rproflim:
        if 'samplestyle' in options and options['samplestyle'] == 'linear':
            step = options['samplelinearscale'] if 'samplelinearscale' in options else 0.5*results['psf fwhm']
        else:
            step = R[-1]*(options['samplegeometricscale'] if 'samplegeometricscale' in options else 0.1)
        ...
        for ang in [1, -1]:
            ...
            for pi, pR in enumerate(R):
                ...
                for oi, oR in enumerate(R):
                    ...
                    sb[key][-1].append((-2.5*np.log10(medflux) + zeropoint + 5*np.log10(options['pixscale'])) if medflux > 0 else 99.999)
    ...
    with open('%s%s_orthogonal_sample.prof' % ((options['saveto'] if 'saveto' in options else ''), options['name']), 'w') as f:
        f.write('R')
        for rd in [1,-1]:
            for ang in [1, -1]:
                for pR in R:
                    f.write(',sb[%.3f:%s90],sbE[%.3f:%s90]' % (rd*pR*options['pixscale'], '+' if ang > 0 else '-', rd*pR*options['pixscale'], '+' if ang > 0 else '-'))
        f.write('\n')
        f.write('arcsec')
        for rd in [1,-1]:
            for ang in [1, -1]:
        ...
        f.write('\n')
        for oi, oR in enumerate(R):
            f.write('%.4f' % (oR*options['pixscale']))
            for rd in [1,-1]:
                for ang in [1, -1]:
                    key = (rd,ang)
                    for pi, pR in enumerate(R):
    ...
    if 'doplot' in options and options['doplot']:
        ...
        for rd in [1,-1]:
            for ang in [1, -1]:
                ...
                norm = matplotlib.colors.Normalize(vmin=0, vmax=R[-1]*options['pixscale'])
                for pi, pR in enumerate(R):
                    if pi % 3 != 0:
                        continue
                    CHOOSE = np.logical_and(np.array(sb[key][pi]) < 99, np.array(sbE[key][pi]) < 1)
                    plt.errorbar(np.array(R)[CHOOSE]*options['pixscale'], np.array(sb[key][pi])[CHOOSE], yerr = np.array(sbE[key][pi])[CHOOSE],
                                 elinewidth = 1, linewidth = 0, marker = '.', markersize = 3, color = autocmap.reversed()(norm(pR*options['pixscale'])))
                plt.xlabel('%s-axis position on line [arcsec]' % ('Major' if 'orthsample_parallel' in options and options['orthsample_parallel'] else 'Minor'))
                ...
                cb1.set_label('%s-axis position of line [arcsec]'  % ('Minor' if 'orthsample_parallel' in options and options['orthsample_parallel'] else 'Major'))
                # plt.colorbar()
                ...
                if not ('nologo' in options and options['nologo']):
                    AddLogo(plt.gcf())
                plt.savefig('%sorthogonal_sample_q%i_%s.jpg' % (options['plotpath'] if 'plotpath' in options else '', count, options['name']), dpi = options['plotdpi'] if 'plotdpi'in options else 300)
                plt.close()
                count += 1
        ...
        outto = np.array(results['prof data']['R'])[CHOOSE][-1]*1.5/options['pixscale']
        ...
        if not ('nologo' in options and options['nologo']):
            AddLogo(plt.gcf())
        plt.savefig('%sorthogonal_sample_lines_%s.jpg' % (options['plotpath'] if 'plotpath' in options else '', options['name']), dpi = options['plotdpi'] if 'plotdpi' in options else 300)
        plt.close()        
