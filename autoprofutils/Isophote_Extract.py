...
def _Generate_Profile(IMG, results, R, E, Ee, PA, PAe, options):
    ...
    if 'ap_doplot' in options and options['ap_doplot']:
        ...
        ax2 = plt.gca().twinx()
        lnlist += ax2.plot(np.array(SBprof_data['R'])[CHOOSE], np.array(SBprof_data['pa'])[CHOOSE]/180, color = 'orange', label = 'PA/180')
        ...
        plt.legend(lnlist, labs, fontsize = 11, loc = 'lower center')
...
def _Generate_Profile_mean(IMG, results, R, E, Ee, PA, PAe, options):
