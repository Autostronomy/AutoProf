...
def EllipseModel_General(IMG, pixscale, name, results, **kwargs):
    ...
    #smooth = SmoothBivariateSpline(X,Y,M,w = 1/np.array(M_e), s = len(X)*2)
    ...
    #MM = smooth(XX.ravel(), YY.ravel(), grid = False)
    #MM = np.reshape(MM,S)
    MM = np.zeros(XX.shape)
    for i in range(XX.shape[0]):
        print(i)
        CHOOSE = abs(Y - YY[i,int(YY.shape[1]/2)]) < (10*results['psf fwhm'])
        K = -((XX[i,:].reshape(XX.shape[1],-1) - X[CHOOSE])**2 + (YY[i,:].reshape(XX.shape[1],-1) - Y[CHOOSE])**2)/((1+np.log(XY_R[CHOOSE]))*results['psf fwhm'])**2
    ...
    # YY /= 1 - ellip[-1]
    # RR = np.sqrt(XX**2 + YY**2)
    # MM[RR > results['fit R'][-1]] = np.interp(RR.ravel(), R, SB).reshape(RR.shape)[RR > results['fit R'][-1]]
    MM = 10**(-(MM - zeropoint - 5*np.log10(pixscale))/2.5)
    MM[RR > R[-1]] = 0
