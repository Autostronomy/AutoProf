...
def Radial_Sample(IMG, results, options):
    ...
    nwedges = options['radsample_nwedges'] if 'radsample_nwedges' in options else 4
    wedgeangles = np.linspace(0, 2*np.pi*(1 - 1./nwedges), nwedges)

    zeropoint = options['zeropoint'] if 'zeropoint' in options else 22.5

    R = np.array(results['prof data']['R'])/options['pixscale']
    SBE = np.array(results['prof data']['SB_e'])
    if 'radsample_variable_pa' in options and options['radsample_variable_pa']:
        pa = np.array(results['prof data']['pa'])*np.pi/180
    else:
        pa = np.ones(len(R))*((options['radsample_pa']*np.pi/180) if 'radsample_pa' in options else results['init pa'])
    dat = IMG - results['background']

    maxwedgewidth = options['radsample_width'] if 'radsample_width' in options else 15.
    maxwedgewidth *= np.pi/180
    if 'radsample_expwidth' in options and options['radsample_expwidth']:
    ...
    for i in range(len(R)):
        ...
        for sa_i in range(len(wedgeangles)):
            ...
            sb[sa_i].append((-2.5*np.log10(medflux) + zeropoint + 5*np.log10(options['pixscale'])) if medflux > 0 else 99.999)
    ...
    if 'doplot' in options and options['doplot']:
        ...
        for sa_i in range(len(wedgeangles)):
            ...
            plt.errorbar(np.array(R)[CHOOSE]*options['pixscale'], np.array(sb[sa_i])[CHOOSE], yerr = np.array(sbE[sa_i])[CHOOSE],
        ...
        if not ('nologo' in options and options['nologo']):
            AddLogo(plt.gcf())
        plt.savefig('%sradial_sample_%s.jpg' % (options['plotpath'] if 'plotpath' in options else '', options['name']), dpi = options['plotdpi'] if 'plotdpi'in options else 300)
        plt.close()
        ...
        if not ('nologo' in options and options['nologo']):
            AddLogo(plt.gcf())
        plt.savefig('%sradial_sample_wedges_%s.jpg' % (options['plotpath'] if 'plotpath' in options else '', options['name']), dpi = options['plotdpi'] if 'plotdpi'in options else 300)
        plt.close()
