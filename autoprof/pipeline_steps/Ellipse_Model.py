...
from autoprofutils.SharedFunctions import AddLogo, autocmap, LSBImage, Rscale_SuperEllipse, Rscale_Fmodes
...
def EllipseModel(IMG, results, options):
    ...
    C = UnivariateSpline(R, np.array(results['prof data']['C'])[CHOOSE], ext = 3, s = 0) if 'C' in results['prof data'] else (lambda r: 2)
    ...
    if not fit_Fmodes is None:
        ...
        for m in range(len(fit_Fmodes)):
            ...
            A.append(UnivariateSpline(R, np.array(results['prof data']['A%i' % fit_Fmodes[m]])[CHOOSE], ext = 3, s = 0))
            Phi.append(UnivariateSpline(R, np.array(results['prof data']['Phi%i' % fit_Fmodes[m]])[CHOOSE] * np.pi/180, ext = 3, s = 0))
    ...
    XX, YY = np.meshgrid(np.arange(ranges[0][1] - ranges[0][0], dtype = np.float32) - results['center']['x'] + float(ranges[0][0]),
                         np.arange(ranges[1][1] - ranges[1][0], dtype = np.float32) - results['center']['y'] + float(ranges[1][0]))
    ...
    for r in reversed(np.logspace(np.log10(R[0]/2),np.log10(R[-1]),int(len(R)*2*(options['ap_ellipsemodel_resolution'] if 'ap_ellipsemodel_resolution' in options else 1)))):
        ...
        if not fit_Fmodes is None:
            Rscale *= np.exp(sum(A[m](r) * np.cos(fit_Fmodes[m] * (theta[WINDOW[0][0]:WINDOW[0][1],WINDOW[1][0]:WINDOW[1][1]] + (Phi[m](r) - pa(r)))) for m in range(len(fit_Fmodes))))
