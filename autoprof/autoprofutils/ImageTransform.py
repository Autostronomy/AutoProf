...
def Resolution(IMG, results, options):
    ...
    *ap_resolution_shape* and perform a mean, median, or max operation
    ...
    if 'ap_resolution_method' in options and options['ap_resolution_method'] == 'max pool':
        M, N = IMG.shape
        K, L = options['ap_resolution_shape']        
        MK = M // K
        NL = N // L
        newIMG = mat[:MK*K, :NL*L].reshape(MK, K, NL, L).max(axis=(1, 3))
    if 'ap_resolution_method' in options and options['ap_resolution_method'] == 'mean pool':
        M, N = IMG.shape
        K, L = options['ap_resolution_shape']        
        MK = M // K
        NL = N // L
        newIMG = mat[:MK*K, :NL*L].reshape(MK, K, NL, L).mean(axis=(1, 3))
    if 'ap_resolution_method' in options and options['ap_resolution_method'] == 'median pool':
        M, N = IMG.shape
        K, L = options['ap_resolution_shape']        
        MK = M // K
        NL = N // L
        newIMG = mat[:MK*K, :NL*L].reshape(MK, K, NL, L).median(axis=(1, 3))
    if 'ap_resolution_method' in options and options['ap_resolution_method'] == 'bicubic':
        newIMG = np.zeros(options['ap_resolution_shape'], dtype = options['ap_resolution_dtype'] if 'ap_resolution_dtype' in options else IMG.dtype)
        XX,YY = np.meshgrid(np.linspace(-0.5,IMG.shape[0]-0.5, options['ap_resolution_shape'][0]), np.linspace(-0.5,IMG.shape[1]-0.5, options['ap_resolution_shape'][1]))
        newIMG = interpolate_bicubic(IMG, XX.ravel(), YY.ravel()).reshape(newIMG.shape)
    else:
        newIMG = np.zeros(options['ap_resolution_shape'], dtype = options['ap_resolution_dtype'] if 'ap_resolution_dtype' in options else IMG.dtype)
        XX,YY = np.meshgrid(np.linspace(-0.5,IMG.shape[0]-0.5, options['ap_resolution_shape'][0]), np.linspace(-0.5,IMG.shape[1]-0.5, options['ap_resolution_shape'][1]))
        for i in range(len(newIMG.shape[0])):
            newIMG[i] = interpolate_Lanczos(IMG, XX[i], YY[i], scale = int(options['ap_iso_interpolate_window']) if 'ap_iso_interpolate_window' in options else 3)
