...
def whenrerun(IMG, results, options):
    ...
    elif not 'onloop' in options: # start by simply re-running the analysis to see if AutoProf got stuck
        return 'head', {'onloop': 1}
    elif options['onloop'] == 1 and (not results['checkfit']['FFT coefficients'] or not results['checkfit']['isophote variability']): # Try smoothing the fit the result was chaotic
        return 'head', {'onloop': 2, 'ap_regularize_scale': 3, 'ap_fit_limit': 5}
    elif options['onloop'] == 1 and not results['checkfit']['Light symmetry']: # Try testing larger area to find center if fit found high asymmetry (possibly stuck on a star)
