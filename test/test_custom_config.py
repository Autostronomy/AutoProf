...
def count_pixel_range(IMG, results, options):
    ...
    count = np.sum(np.logical_and(IMG > options['ap_mycountrange_low'], IMG < options['ap_mycountrange_high']))

    logging.info('%s: counted %i pixels in custom range' % (options['ap_name'],count))
    
    return IMG, {'auxfile count pixels in range': 'In range from %.2f to %.2f there were %i pixels' % (options['ap_mycountrange_low'], options['ap_mycountrange_low'], count)}


ap_new_pipeline_steps = {'head': ['mask badpixels', 'background', 'count pixel range', 'custom writebackground']}
ap_new_pipeline_methods = {'custom writebackground': mywriteoutput,
